...
## Added 2015-10-28
## Support for different ui cultures when converting DateTime values
    ...
    foreach ($kvp in $inputLogHash.GetEnumerator())
    {
        ...
        if ($log.Count -ge 2)
        {
            for ($j = 1; $j -lt $log.Count; $j++)
            {
                ...
                # Changed 2015-10-28, Thomas Stensitzki
                # Original $previousDate = [DateTime]::Parse($previousSample[1])
                $previousDate = [DateTime]::Parse($previousSample[1],$uiCulture)

               
                $currentSample = $log[$j].Split(',')
                # Changed 2015-10-28, Thomas Stensitzki
                # Original $currentDate = [DateTime]::Parse($currentSample[1],$uiCulture)
                $currentDate = [DateTime]::Parse($currentSample[1],$uiCulture)
